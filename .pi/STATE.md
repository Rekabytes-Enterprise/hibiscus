...
## Snapshot — incoming image-echo checkpoint, 2026-09-25 (UTC)

- Branch: `dev`; manifest version: `0.1.7`; last pushed HEAD **`9636c39`**. The user requested a local checkpoint commit for incoming user `message_start` image-block `data` omission. Delivered text/count/metadata remain available; assistant messages, user message ends, unknown blocks/events, history, approvals, errors and settlement remain fully decoded. Pi persistence and outbound JSONL image payloads are unchanged. The requested binary digest was removed from current tracked records; local profiling reports remain outside Git. No push, version bump or tag was requested; remote CI for this source remains pending. Earlier CI [36174873637](https://github.com/Rekabytes-Enterprise/hibiscus/actions/runs/36174873637) passed on the previous decoder source.
...
The checks below were rerun on Linux after image-echo decoding, against the source/test/benchmark fingerprint below. Full release-mode Rust testing was not performed; targeted release PTY suites were run.

| Check | Result and scope |
| --- | --- |
| `cargo test --locked` | Local pass: 101 unit + 77 integration tests. Remote Ubuntu/macOS results apply to older source, not this change; these are mock/PTY scenarios, not real-provider acceptance tests. |
| `cargo clippy --locked --all-targets -- -D warnings` | Pass; static checks only. |
| `cargo fmt --all -- --check` | Pass. |
| `cargo build --release --locked` | Pass; local profiler confirmed the same release binary was used for all three image-echo experiments. No binary digest is stored in this record; older binary digests already committed in Git history are not rewritten by this task. |
| `python3 tests/profile_memory.py` | Pass: six tests for counters, framing, metrics and failed-report handling. |
| `python3 scripts/profile-memory.py --scenario image-send-max-default --repeats 3 --output …` | Complete: three fresh synthetic experiments; earlier 33-run baseline and decoder/shared-image follow-ups remain historical in the memory report. |
| Allocation benchmark | Previously passed for shared images; not rerun as part of this decoder verification. |
| `cargo test --locked --release --test steering_tty --test image_paste --test rpc_pressure --test performance_tty --test goal_tty --test approval_tty --test error_recovery` | Pass: 28 targeted release PTY/integration tests for image/queue delivery, pressure, errors, approval, goals and rendering. |
...
Source/test/benchmark fingerprint: `575cc71f22dd803d011a3aac52b474d91e5ce85fd01955d0ae7cf87eb7cf7aa6`.
...
1. Retest real Pi image delivery and macOS CI for this source before release. Next performance candidate: measure event-loop wake latency before changing the 40 ms polling loop; preserve Esc responsiveness, queue ordering, protocol boundaries and terminal handoff.
