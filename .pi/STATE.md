...
## Snapshot — quiet-run input wake dev push verified, 2026-09-25 (UTC)

- Branch: `dev`; manifest version: `0.1.7`. Pushed incoming image-echo checkpoint **`0d95321`** and shared input/RPC wake follow-up **`685c413`** to `origin/dev`. GitHub CI [36177585894](https://github.com/Rekabytes-Enterprise/hibiscus/actions/runs/36177585894) for `685c413` passed macOS 14 tests, Ubuntu tests and formatting/Clippy/installer checks. Wake notifications do not change protocol IDs, ordering, acceptance/delivery/settlement, authorization or Pi ownership; 40 ms remains as an animation/recovery fallback. This records-only update leaves the tested source fingerprint unchanged. No version bump/tag. The requested binary digest remains absent from current tracked records; local profiling reports stay outside Git.
...
| --- | --- |
| `cargo test --locked` | Local pass: 102 unit + 77 integration tests; remote Ubuntu/macOS jobs passed on `685c413`. These are mock/PTY scenarios, not real-provider acceptance tests. |
...
1. Retest real Pi image delivery and input wake behavior, including Esc, steering/follow-up, resize, cursor visibility and login handoff, on the intended terminal. Remote Ubuntu/macOS CI passed on `685c413`, but CI cannot establish physical-terminal behavior.
