...
| Manual `/compact` | Wait for the matching Pi `compact` response after the model-generated summary (not the metadata deadline or `agent_settled`). A silent 10-minute idle deadline and a 10-second Esc cancellation deadline disconnect with uncertain outcome. Failed/cancelled compaction remains Pi-owned; do not replay or claim completion. |
