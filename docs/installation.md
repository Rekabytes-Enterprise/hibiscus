...
Install a specific release with `HIBISCUS_VERSION=v0.1.4`. For a private GitHub repository, export a token that can read repository releases, and fetch the script through GitHub's authenticated API:
...
The release version in `Cargo.toml`, `Cargo.lock`, the docs, and Git tag must match. Before tagging, use `sh scripts/bump-version.sh 0.1.4` (substitute the intended version), review the diff, test, commit, and merge the bump into the intended release branch. The script does **not** create or move tags. Push an annotated version tag pointing at that exact release commit:

```sh
git tag -a v0.1.4 -m "Hibiscus v0.1.4"
