...
- The mailbox bounds both queued record count and allocated byte capacity. The consumer parses each record only when it dequeues it. For `tool_execution_update`, only the unused `partialResult` subtree is omitted from the resulting JSON value; it is still validated as JSON (including escaped strings). Every other field/event retains its normal decoding behavior.
